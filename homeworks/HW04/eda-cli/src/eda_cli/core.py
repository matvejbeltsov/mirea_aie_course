...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # константые признаки
    flags["has_constant_columns"] = any(col.unique is not None and col.unique <= 1 for col in summary.columns)



    # высокая кардинальность категориальных признаков
    high_cardinality_threshold = min(100, int(0.3 * summary.n_rows))
    flags["has_high_cardinality_categoricals"] = any(
            col.unique is not None
            and col.unique > high_cardinality_threshold
            and (
                "object" in (col.dtype or "").lower()
                or "category" in (col.dtype or "").lower()
                or "string" in (col.dtype or "").lower()
            )
