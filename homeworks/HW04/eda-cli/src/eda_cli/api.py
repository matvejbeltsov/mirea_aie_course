...
class QualityFlagsResponse(BaseModel):
    """Ответ: полный набор булевых флагов качества данных."""
    flags: dict[str, bool] = Field(..., description="Булевы эвристики качества датасета")
...
@app.post(
    ...
    tags=["quality"],
    summary="Полный набор булевых флагов качества по CSV-файлу (включая HW03 эвристики)",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает полный набор булевых флагов качества, включая новые из HW03.
    """
    ...
    # Возвращаем полный набор булевых флагов (включая has_constant_columns и т.п.)
    flags_bool: dict[str, bool] = {
        key: bool(value)
        for key, value in flags_all.items()
        if isinstance(value, bool)
